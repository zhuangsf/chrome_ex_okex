...
;(function ($) {
    ...
    * }
        ...
        var xhr = $.ajax({
            ...
            success: function (response) {
                ...
                if (response.code === 0) {
                    ...
                    _cfg.error ? _cfg.error(200, response.msg) : '';
                }
            },
            error: function (xhr, textStatus, err) {
                _cfg.error ? _cfg.error(xhr.status, '') : '';
            }
        });
        return xhr;
    };
    var ajax = {
        get: function (config) {
            return request({type: 'get'}, config);
        },
        post: function (config) {
            return request({type: 'post'}, config);
        }
    };
...
;(function () {
    var URL = {
        getRankingList: '/v2/support/activity/vote/rankingList',
        getInvestors: '/v2/support/activity/vote/investors',
        getInvestorDetail: '/v2/support/activity/vote/investor',
        getProjectIntroduce: '/v2/support/activity/vote/project',
        getCanUseOkb: '/v2/support/activity/vote/user/availableOkb',
        getRecord: '/v2/support/activity/vote/user/record',
        postVoteNum: '/v2/support/activity/vote/support/user',
        getShareImg:'/v2/support/activity/vote/shareImage',
        postInvestorSupport:'/v2/support/activity/vote/support/investor',
        postReturnTickets: '/v2/support/activity/vote/refund/user',
        getNikeName :'/v3/users/security/profile'
    };

    if (location.href.indexOf('localhost') > -1) {
        for (var k in URL) {
            if (URL.hasOwnProperty(k)) {
                //URL[k] = 'http://192.168.10.104:8104' + URL[k];
                URL[k] = 'http://localhost:3004' + URL[k];
            }
        }
    }
...
(function (window, $) {
    $.fn.pop = function (options) {
        var defaults = {
            animation: 'fadeAndPop', //fadeAndPop动画类型
            animationSpeed: 300, //动画持续时间
            closeOnBgClick: true, //点击遮盖层是否关闭弹出框
            closeBtnClass: 'pop-close', //关闭按钮的类名
            closefn: function () {
            }
        };
        ...
        return this.each(function () {
            ...
            var modal = $(this),
                ...
                locked = false,
                modeMask = $('.pop-modal-bg');

            //创建遮盖层
            if (modeMask.length == 0) {
                modeMask = $('<canvas class="pop-modal-bg" id="popCanvas" style="opacity: 0.85" />').insertAfter(modal);
            ...
            modal.bind('open', function () {
                modeMask.unbind('click.modalEvent');
                $('.' + options.closeBtnClass).unbind('click.modalEvent');
                if (!locked) {
                    lockModal();
                    modal.css({
                        "top": '50%',
                    ...
                    modeMask.fadeIn(options.animationSpeed / 2);
            ...
            modal.bind('close', function () {
                if (!locked) {
                    lockModal();
                    modeMask.delay(options.animationSpeed)
                ...
                typeof options.closefn == 'function' ? options.closefn() : function () {
                }();
                modal.unbind('close');
            });
            ...
            //监听关闭事件
            var closeButton = $('.' + options.closeBtnClass).bind('click.modalEvent', function () {
                modal.trigger('close');
            });

            if (options.closeOnBgClick) {
                modeMask.css({"cursor": "pointer"});
                modeMask.bind('click.modalEvent', function () {
                    modal.trigger('close');
                });
            }
